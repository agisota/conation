...
//! injection into containers (via the `CONATION_ENV_FILE` indirection in the
//! `x-common-env` anchor).
...
pub struct ResolvedEnv {
    ...
    /// Whether the generated compose environment changed since the last
    /// resolution for this instance. The value is compared as rendered bytes,
    /// so key additions/removals and value changes are all significant without
    /// retaining a second copy of any secret in stack state.
    pub generated_env_changed: bool,
    /// SHA-256 of the canonical generated file. Stack update records this only
    /// after the relevant containers have been recreated, so a failed update
    /// remains detectable on its retry without retaining environment values.
    pub generated_env_fingerprint: String,
}
...
pub fn resolve(
    ...
    let generated_env_changed = write_dotenv(&generated_path, &env)
        .with_context(|| format!("writing {}", generated_path.display()))?;
    let generated_env_fingerprint = generated_env_fingerprint(&env);
...
/// Write the merged map as a docker-compose `env_file`, returning whether its
/// rendered content differs from the previous generated file. This is an
/// *emitter*, not a dotenv parser: no crate writes compose's env-file format
/// (dotenvy and friends only read), and the values come from Doppler so they
/// can contain spaces, quotes, and newlines. Values that need it are
/// double-quoted with escapes so compose's `env_file` parser round-trips them.
///
/// A missing generated file is drift: a running stack has no trustworthy
/// record of the container environment in that case and update must reconcile
/// the app services once.
fn write_dotenv(path: &Path, env: &BTreeMap<String, String>) -> Result<bool> {
    let rendered = render_dotenv(env);
    let changed = generated_env_changed(std::fs::read(path), rendered.as_bytes())?;
    if changed {
...
/// Render the merged map as deterministic Compose dotenv content. `BTreeMap`
/// ordering makes a byte comparison a complete, secret-safe drift detector.
...
/// A secret-free marker for the exact canonical rendering. This is stored only
/// after a stack has applied the environment to its containers; it is not
/// logged or used as a substitute for the generated file itself.
fn generated_env_fingerprint(env: &BTreeMap<String, String>) -> String {
    let mut hasher = Sha256::new();
    hasher.update(render_dotenv(env).as_bytes());
    format!("{:x}", hasher.finalize())
}

/// Compare the prior generated bytes to the next rendering without ever
/// parsing or printing values. Any read failure other than a missing file is
/// surfaced rather than silently replacing a file we could not inspect.
fn generated_env_changed(previous: Result<Vec<u8>, std::io::Error>, next: &[u8]) -> Result<bool> {
    match previous {
        Ok(previous) => Ok(previous != next),
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => Ok(true),
        Err(error) => Err(error).context("reading previous generated compose environment"),
    }
}
