use crate::{
    ...
    model::response::documents::get::GetDocumentSearchResponse,
};
...
pub async fn get_document_list_handler(
    ...
    let document_search_response_date: Vec<GetDocumentListResult> =
        match conation_db_client::document::get_document_list(
            ctx.db.clone(),
        ...
        .await
        {
            ...
            Err(e) => {
                tracing::error!(error=?e, user_id=?user.authorization.user.conation_user_id, "unable to get document search");
