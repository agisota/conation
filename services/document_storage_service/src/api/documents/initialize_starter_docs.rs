...
pub(in crate::api) const HOW_TO_GUIDE_NAME: &str = "Знакомство с Conation";
...
const STARTER_TASKS: [StarterTask; 3] = [
    ...
    StarterTask {
        name: "Расширенные возможности задач",
    ...
    StarterTask {
        name: "Пример работы с задачами в Conation",
...
const DOCS_TAG_LABEL: &str = "документы";
...
/// Find-or-create the user's personal starter-document tag, returning its
/// `(property_definition_id, option_id)`. Provisions the personal tag set on
/// first use. Tags are matched by label, so a pre-existing localized tag (or
/// a rerun that got this far before failing) is reused, never duplicated.
...
/// Creates the user's starter content — the Russian-first Conation guide
...
pub async fn handler(
    ...
    let system_for_user = Attribution::delegated(
        Actor::new_from_bot(bot_id::CONATION_SYSTEM_BOT_ID),
        user_id.clone(),
    );
