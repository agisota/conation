...
pub async fn get_instructions_handler(
    ...
    match conation_db_client::instructions::get::get_instructions_document(
        &ctx.db,
    ...
    .await
    {
        ...
        Err(err) => {
            tracing::error!(error=?err, user_id=%user_context.authorization.user.conation_user_id, "failed to get instructions document");
