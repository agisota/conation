...
const MODEL_PROXY_PATH: &str = "/conation-model-proxy/v1";
...
impl Default for ProxyConfig {
    fn default() -> Self {
        Self::from_values(
            "https://model-proxy.invalid/conation-model-proxy/v1",
            "",
            None,
...
impl ProxyConfig {
    fn from_env() -> Result<Self, String> {
        let upstream_base_url = std::env::var(MODEL_PROXY_URL_VARIABLE)
            .unwrap_or_else(|_| "https://model-proxy.invalid/conation-model-proxy/v1".to_owned());
...
fn parse_upstream_base_url(value: &str) -> Result<Url, String> {
    ...
    if url.path().trim_end_matches('/') != MODEL_PROXY_PATH {
        return Err("CONATION_MODEL_PROXY_URL must end in /conation-model-proxy/v1".to_owned());
    }
    url.set_path("/conation-model-proxy/v1/");
    Ok(url)
}
