...
use agent_harness::domain::model::{
    ...
    SessionRepository, SpawnContainer, StaticFileLinks,
};
use agent_harness::domain::model_load::AgentModelsServiceImpl;
use agent_harness::domain::ports::{AgentRuntimeDirectory as _, ContainerManager};
...
use agent_session::domain::model::{AgentMcpServers, AgentSessionId, ReplicaId, SandboxSize};
...
///
/// Empty `CURSOR_REPO_URL` boots unarmed: sandboxed and in-process sessions
/// stay alive, while spawn and resume fail instead of substituting a shared
/// repository.
#[derive(Clone)]
enum CursorRuntime<Manager> {
    Armed(Manager),
    Unarmed,
}
...
impl<Manager> ContainerManager for CursorRuntime<Manager>
where
    Manager: ContainerManager,
{
    type Transport = Manager::Transport;

    async fn spawn(
        &self,
        command: SpawnContainer,
    ) -> agent_harness::domain::error::Result<RuntimeAttachment<Self::Transport>> {
        match self {
...
async fn run() -> anyhow::Result<()> {
    ...
    // The in-process Conation(new) bot is a compile-time identity. Production
    // stays off until its AI tool config lands - `build_tool_service_context_from_env`
    // is fatal, so turning it on without that config would refuse to boot.
    ...
    // `conation` slug, then the owner's Pipedream connections. The credential
    // is signed inline with the same key authentication_service holds; what this
    ...
    let (inmem, inmem_model_engine) = if inmem_bot.is_some() {
        ...
        (Some(inmem), Some(inmem_model_engine))
    } else {
        (None, None)
    };
    ...
    let cursor_built = CursorContainerManager::new(
        cursor_keys.clone(),
        cursor_api_base_url(),
        session_repo.clone(),
    ...
    let cursor_manager = match config.cursor_repo_for_spawn() {
        Ok(_) => CursorRuntime::Armed(cursor_built),
        Err(_) => {
            tracing::warn!(
    ...
    // Empty by default: a hardcoded repository would force every team onto
    // the same clone. Spawn fails later when neither the request nor this
    // value names a repository.
    let repo_url = SessionRepository::parse(config.harness_repo_url.trim());
    if repo_url.is_none() {
        tracing::warn!(
    ...
    // when an in-memory bot is served; explicitly selected coding agents
    // keep their configured runtimes.
    ;
