...
pub struct SupportTeam {
    ...
    executive: MacroUserIdStr<'static>,
    technical_lead: MacroUserIdStr<'static>,
}
...
impl SupportTeam {
    ...
    pub fn try_from_emails(
        ...
        Ok(Self {
            ...
            executive: support_user("executive", executive)?,
            technical_lead: support_user("technical lead", technical_lead)?,
        })
    }
...
Если у вас появятся вопросы, предложения или вы найдёте ошибку — напишите нам здесь.",
        user_mention(&support_team.executive)?,
        user_mention(&support_team.technical_lead)?,
    );
