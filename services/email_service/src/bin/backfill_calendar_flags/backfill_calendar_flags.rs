//! # Calendar Flag Backfill Utility
//!
//! Backfills the denormalized `email_threads.has_calendar_attachment` column:
//! for each user, flags every thread that has at least one iCalendar
//! attachment (`*.ics` filename or `text/calendar` / `application/ics` mime).
//! Only ever sets the flag to true — steady-state maintenance (including
//! clearing it on deletes) is handled by the ingest and delete paths in
//! `email_db_client`.
//!
