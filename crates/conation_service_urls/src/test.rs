...
fn defaults_are_selected_by_environment() {
    with_mock_override_env(missing_override, || {
        ...
        assert_eq!(
            TestServiceUrl::new_for_environment(conation_env::Environment::Develop)
                .unwrap()
                .as_ref(),
            "https://test-dev.macro.com",
        );
        assert_eq!(
            TestServiceUrl::new_for_environment(conation_env::Environment::Production)
                .unwrap()
                .as_ref(),
...
fn grouped_conation_resolves_all_service_urls() {
    let service_urls = with_mock_override_env(mock_group_overrides, || {
        TestServiceUrls::new_for_environment(conation_env::Environment::Develop).unwrap()
    });
