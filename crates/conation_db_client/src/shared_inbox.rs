...
//! link onto it, and grant both connectors access via `conation_user_links` edges.
...
pub struct PromotedSharedInbox {
    /// The conation_id minted for the mailbox. The re-homed link and both edges point at it.
    pub mailbox_conation_id: String,
    /// The uuid minted as the mailbox's `conation_user.id`. Grant relocation creates the
...
/// and grants both the original owner and the new connector access via `conation_user_links`
/// edges. Because the minted conation_id embeds the mailbox email, the re-homed link's email
/// matches its conation_id — i.e. it is no longer an inbox-only secondary but a shared user.
///
...
pub async fn promote_link_to_shared(
    ...
    original_owner_conation_id: &str,
    new_connector_conation_id: &str,
    mailbox_email: &str,
    ...
    sqlx::query!(
        r#"
        INSERT INTO "User" (id, email, conation_user_id, "organizationId")
        VALUES ($1, $2, $3, $4)
        "#,
        &mailbox_conation_id,
        mailbox_email,
    ...
    // Mark this conation_id as a promoted shared mailbox so teardown can distinguish it from a
    // real account: the last delegate removing access tears it down rather than orphaning it.
    sqlx::query!(
        r#"
        INSERT INTO promoted_shared_mailboxes (conation_id)
        VALUES ($1)
        ON CONFLICT (conation_id) DO NOTHING
        "#,
        &mailbox_conation_id,
    )
    .execute(&mut *conn)
    ...
    let rehomed = sqlx::query!(
        r#"
        UPDATE email_links
        SET conation_id = $1, updated_at = NOW()
        WHERE id = $2
        "#,
        &mailbox_conation_id,
        existing_link_id,
    ...
    crate::conation_user_links::insert_edge(
        &mut *conn,
        original_owner_conation_id,
        &mailbox_conation_id,
        existing_link_id,
    )
    .await?;
...
/// Whether `conation_id` is a mailbox minted by shared-inbox promotion (no human owner),
...
pub async fn is_promoted_shared_mailbox(
    ...
    let exists = sqlx::query_scalar!(
        r#"
        SELECT EXISTS(
            SELECT 1 FROM promoted_shared_mailboxes WHERE conation_id = $1
        ) AS "exists!"
        "#,
        conation_id,
    )
...
/// been torn down. Removing the `User` row cascades its `conation_user_links` edges and the
/// `promoted_shared_mailboxes` marker; the backing `conation_user` row is removed explicitly.
/// No-op (returning `None`) when `conation_id` is not a promoted mailbox; otherwise returns
/// the deleted `conation_user.id`, which callers can match against a link's
...
pub async fn delete_promoted_mailbox_user(
    ...
    let row = sqlx::query!(
        ...
        DELETE FROM "User"
        WHERE id = $1
          AND EXISTS (SELECT 1 FROM promoted_shared_mailboxes WHERE conation_id = $1)
        RETURNING conation_user_id
        "#,
        conation_id,
    )
    ...
    sqlx::query!(r#"DELETE FROM conation_user WHERE id = $1"#, row.conation_user_id)
        .execute(&mut *conn)
        .await?;

    Ok(Some(row.conation_user_id))
}
