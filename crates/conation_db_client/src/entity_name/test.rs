...
async fn test_get_entity_name_and_owner(pool: sqlx::Pool<sqlx::Postgres>) -> anyhow::Result<()> {
    let entity_id = uuid::Uuid::parse_str("11111111-1111-1111-1111-111111111111")?;
    let entity_name =
        get_entity_name_and_owner(&pool, &entity_id, &SearchEntityType::Documents).await?;
    assert_eq!(
        entity_name,
        (
    ...
    let entity_name =
        get_entity_name_and_owner(&pool, &entity_id, &SearchEntityType::Emails).await?;
    assert_eq!(
        entity_name,
        (
    ...
    let result = get_entity_name_and_owner(&pool, &entity_id, &SearchEntityType::Projects).await;
