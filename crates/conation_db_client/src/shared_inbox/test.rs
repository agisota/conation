...
const OWNER: &str = "conation|alice@company.test";
const CONNECTOR: &str = "conation|bob@company.test";
...
async fn promote_dedups_to_single_link_with_two_edges(pool: Pool<Postgres>) -> anyhow::Result<()> {
    ...
    let mailbox_macro_id = format!("conation|{MAILBOX_EMAIL}");
    ...
    // is_primary is generated as `link.email == the principal ID's email`. The minted macro_id
