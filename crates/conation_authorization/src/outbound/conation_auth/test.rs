...
const MACRO_USER_ID: &str = "conation|user@example.com";
...
fn conation_auth_cause(report: &Report<MacroAuthorizationError>) -> &MacroAuthError {
    report
        .iter_sub_reports()
        .find_map(|cause| cause.downcast_current_context::<MacroAuthError>())
        .expect("validation report should retain its conation_auth cause")
}
...
#[test]
fn maps_conation_api_token_claims() {
    let identity = identity_from_token(JwtToken::ConationApiToken(ConationApiToken {
