...
fn expand_conation_config(input: DeriveInput) -> syn::Result<TokenStream2> {
    ...
    for (index, field) in fields.into_iter().enumerate() {
        ...
        let default = conation_config_default(&field.attrs)?;
        ...
        match default {
            Some(default) => quote! {
                __MacroConfigField::#variant => {
                    if #ident.is_some() {
                        return Err(<V::Error as ::conation_config::__serde::de::Error>::duplicate_field(#key));
                    }
                    #ident = Some(
        ...
        match default {
            ...
            None => quote! {
                let #ident = match #ident {
                    Some(value) => value,
                    None => ::conation_config::__deserialize_missing_field::<#ty, V::Error>(#key)?,
                };
            },
        }
    });
    ...
    // Fields covered by FromRef each get a `conation_config::FromRef<Struct>` impl keyed on the
    ...
    let from_ref_impls = field_data.iter().filter(|field| field.from_ref).map(|field| {
        ...
        let ty = &field.ty;
        quote! {
            impl #from_ref_impl_generics ::conation_config::FromRef<#struct_name #ty_generics> for #ty
        ...
        quote! {
            impl #inherent_impl_generics #struct_name #inherent_ty_generics #inherent_where_clause {
                ...
                pub async fn resolve_remote_secrets<__MacroConfigSecretManager>(
                    self,
                    environment: ::conation_config::__conation_env::Environment,
                    secret_manager: &__MacroConfigSecretManager,
                ) -> ::core::result::Result<
                    Self,
                    <__MacroConfigSecretManager as ::conation_config::__remote_env_var::SecretManager>::Err,
                >
                where
                    __MacroConfigSecretManager: ::conation_config::__remote_env_var::SecretManager,
                {
    ...
    Ok(quote! {
        impl #impl_generics ::conation_config::__serde::Deserialize<'de> for #struct_name #ty_generics #where_clause {
            fn deserialize<__D>(deserializer: __D) -> Result<Self, __D::Error>
            where
                __D: ::conation_config::__serde::Deserializer<'de>,
            {
                ...
                impl<'de> ::conation_config::__serde::Deserialize<'de> for __MacroConfigField {
                    fn deserialize<__D>(deserializer: __D) -> Result<Self, __D::Error>
                    where
                        __D: ::conation_config::__serde::Deserializer<'de>,
                    {
                        ...
                        impl<'de> ::conation_config::__serde::de::Visitor<'de> for __MacroConfigFieldVisitor {
                            ...
                            where
                                __E: ::conation_config::__serde::de::Error,
                            {
                                match value {
                                    #(#field_matches)*
                ...
                impl<'de> ::conation_config::__serde::de::Visitor<'de> for __MacroConfigVisitor {
                    ...
                    where
                        V: ::conation_config::__serde::de::MapAccess<'de>,
                    {
                        #(#initializers)*
...
/// `conation_config::FromRef<Struct>` impl for its type.
...
/// `conation_config::FromRef<Struct>` impl. Requires all field types to be distinct.
...
fn conation_config_default(attrs: &[syn::Attribute]) -> syn::Result<Option<TokenStream2>> {
    let mut default = None;

    for attr in attrs {
        if attr.path().is_ident("conation_config_default") {
            if default.is_some() {
                return Err(syn::Error::new_spanned(
                    attr,
                    "duplicate conation_config_default attribute",
                ));
            }
