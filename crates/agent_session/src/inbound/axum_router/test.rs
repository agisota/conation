...
use conation_authorization::{
    ...
    BotScope, HARNESS_FOR_CONATION_USER_ID_HEADER, HARNESS_TOKEN_HEADER, HarnessAuthentication,
...
impl HarnessAuthorizer for SelfHarnessAuthorizer {
    async fn authorize_harness(
        ...
        let user_id = conation_user_id.as_ref().to_owned();
...
fn as_harness_for(user: &str, request_body: String) -> Request<Body> {
    Request::post("/")
        .header(header::CONTENT_TYPE, "application/json")
        .header(HARNESS_TOKEN_HEADER, HARNESS_TOKEN)
        .header(HARNESS_FOR_CONATION_USER_ID_HEADER, user)
        .body(Body::from(request_body))
        .unwrap()
}
