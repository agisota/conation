...
1. **Conation workspace** — `ListEntities` sorted by `recently_updated` with a `df` updatedAt window for the day: documents edited, projects touched, calls joined. Query tasks separately with `df` subtype task plus the same window, filtering Status = Completed and Assignees = the user, to find tasks they finished.
...
Link each event to its Conation item or external URL. After the timeline, add a two-or-three-sentence summary of the day's main threads of work — what the day was mostly about, not a restatement of every line. If a source returned nothing, note it in one line ("No GitHub activity yesterday").
"##;

static INTENT: &str = "Day-recap requests produce a chronological, linked timeline of the user's own actions \
     across Conation, GitHub, and connected integrations, grouped by part of day and followed by a short \
