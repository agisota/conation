...
pub async fn insert_thread_share_permissions(
    ...
    let permission =
        conation_db_client::share_permission::get::get_email_thread_permission(db, thread_id)
            .await
    ...
    let share_permission = SharePermissionV2 {
        id: conation_uuid::generate_uuid_v7().to_string(),
        link_share: None,
    ...
    let permission = conation_db_client::share_permission::create::create_thread_permission(
        &mut tx,
        owner_id.copied(),
        thread_id,
        &share_permission,
    ...
    entity_access_db_utils::insert_entity_access_row(
        &mut tx,
        &conation_uuid::string_to_uuid(thread_id).unwrap(),
