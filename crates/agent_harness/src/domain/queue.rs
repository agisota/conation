...
pub struct QueuedEntry {
    ...
    /// The chip message, once posted. Set when the chip is announced — at
    /// enqueue if the session is busy, otherwise at the dispatch that posts
    /// it — and carried through a requeue, so a retry does not post a
    /// second chip.
