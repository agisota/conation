...
async fn serve_proxy(replies: impl IntoIterator<Item = MockReply>) -> (SocketAddr, MockUpstream) {
    ...
    let proxy = ProxyConfig::from_values(
        &format!("http://{upstream_address}/openai/v1"),
...
async fn retryable_provider_failures_advance_models_in_order_without_client_secret_headers() {
    ...
    assert!(
        calls
            .iter()
            .all(|call| call.path == "/openai/v1/chat/completions")
    );
    assert!(
        calls
            .iter()
...
#[tokio::test]
async fn only_chat_completions_post_is_exposed_to_opencode() {
    let (proxy_address, upstream) = serve_proxy([]).await;
    ...
    assert_eq!(
        client
            ...
            .send()
            .await
            .expect("request loopback proxy")
            .status(),
        StatusCode::METHOD_NOT_ALLOWED
    );
    assert_eq!(
        client
            .post(format!("http://{proxy_address}/v1/models"))
            .send()
            .await
            .expect("request loopback proxy")
            .status(),
...
fn fallback_proxy_configuration_rejects_unsafe_overrides() {
    ...
    assert!(
        ProxyConfig::from_values(
            "https://egress.example/openai/v1",
