...
async fn test_inboxes_for_macro_id_includes_own_and_delegated(
    ...
    assert_eq!(
        detailed_emails,
        emails.into_iter().map(str::to_owned).collect(),
        "enriched inbox details must preserve owned and delegated scoping"
    );
...
async fn user_inbox_details_maps_enriched_owned_inbox(pool: Pool<Postgres>) -> anyhow::Result<()> {
    ...
    sqlx::query!(
        ...
        VALUES ($1, $2, 'fa-user-1', 'InProgress')
        "#,
        Uuid::from_u128(901),
        link_id
    )
    ...
    sqlx::query!(
        r#"
        INSERT INTO email_contacts (id, link_id, email_address, sfs_photo_url)
        VALUES ($1, $2, 'USER1@test.com', 'https://example.com/inbox.png')
        "#,
        Uuid::from_u128(902),
        link_id
    )
    ...
    let inbox = &details[0];
    assert_eq!(inbox.id, link_id);
    assert_eq!(inbox.email_address.0.as_ref(), "user1@test.com");
    assert_eq!(inbox.provider, UserProvider::Gmail);
    assert!(inbox.is_sync_active);
    assert!(inbox.needs_reauth);
    assert!(inbox.is_primary);
    assert_eq!(
        inbox.latest_backfill_status,
        Some(EmailBackfillStatus::InProgress)
    );
    assert!(inbox.settings.signature_on_replies_forwards);
    assert_eq!(inbox.settings.signature.as_deref(), Some("<p>Regards</p>"));
    assert_eq!(
...
#[sqlx::test(migrator = "MACRO_DB_MIGRATIONS")]
async fn test_link_by_fusionauth_email_provider_decodes_stalwart(
    pool: Pool<Postgres>,
) -> anyhow::Result<()> {
    sqlx::query(
        r#"
        ...
        VALUES (
            'ce000000-0000-0000-0000-000000000001'::uuid,
            'stalwart-user',
            'stalwart@conation.test',
            'stalwart-user'
        )
        "#,
    )
    ...
    sqlx::query(
        ...
        VALUES (
            'conation|stalwart@conation.test',
            'stalwart@conation.test',
            'ce000000-0000-0000-0000-000000000001'::uuid
        )
        "#,
    ...
    sqlx::query(
        ...
        INSERT INTO email_links (
            ...
            'ce000000-0000-0000-0000-000000000002'::uuid,
            'conation|stalwart@conation.test',
            'fa-stalwart-user',
            'stalwart@conation.test',
            'STALWART'
    ...
    let link = EmailPgRepo::new(pool)
        ...
        .await?
        .expect("Stalwart link should decode through the outbound adapter");

    assert_eq!(link.provider, UserProvider::Stalwart);
    assert_eq!(link.macro_id.as_ref(), "conation|stalwart@conation.test");
    Ok(())
}
