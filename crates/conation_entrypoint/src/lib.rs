...
/// `APP_SECRETS_JSON` (the way the rest of our config is injected, see `conation_env_var`) is ignored
/// and our tracing filter ends up wrong. This reads `RUST_LOG` the same way `conation_env_var` does —
...
/// `conation_tower_layers::CustomOnFailure`'s "response failed", or an
...
fn init_opentelemetry(deployment_environment: String) -> SdkTracerProvider {
    // W3C trace-context propagation: lets conation_tower_layers parent request
