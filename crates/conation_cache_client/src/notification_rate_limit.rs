...
impl MacroCache {
    ...
    pub async fn get_channel_invite_rate_limit(&self, email: &str) -> anyhow::Result<Option<u64>> {
        ...
        conation_redis::get::get_optional::<u64>(
            &self.inner,
            &conation_rate_limit_channel_invite!(normalized_email),
        )
        .await
    }
    ...
    pub async fn increment_channel_invite_rate_limit(
        ...
        let normalized_email = email_validator::normalize_email(email)
            .with_context(|| format!("unable to normalize email {}", email))?;
        conation_redis::incr::incr_with_expiry(
            &self.inner,
            &conation_rate_limit_channel_invite!(normalized_email),
            expiry_seconds,
        )
    ...
    pub async fn get_invite_to_team_rate_limit(&self, email: &str) -> anyhow::Result<Option<u64>> {
        ...
        conation_redis::get::get_optional::<u64>(
            &self.inner,
            &conation_rate_limit_invite_to_team!(normalized_email),
        )
        .await
    }
    ...
    pub async fn get_channel_invited_rate_limit(
        ...
        conation_redis::get::get_optional::<u64>(
            &self.inner,
            &conation_rate_limit_channel_invited!(normalized_email, normalized_invited_by_email),
        )
        .await
    }
    ...
    pub async fn get_channel_invited_rate_limit_bulk(
        ...
        let keys = normalized_emails
            .iter()
            .map(|email| conation_rate_limit_channel_invited!(email, normalized_invited_by_email))
            .collect::<Vec<String>>();

        let limits = conation_redis::get::get_multiple::<u64>(&self.inner, &keys).await?;
    ...
    pub async fn increment_channel_invited_rate_limit_bulk(
        ...
        conation_redis::incr::incr_with_expiry_bulk(&self.inner, &keys, expiry_seconds).await
    }
}
