...
impl MacroCache {
    ...
    pub async fn delete_user(&self, user_id: &str) -> anyhow::Result<()> {
        ...
        let session_id = conation_redis::get::get_optional::<String>(&self.inner, user_id).await?;
        ...
        conation_redis::delete::delete_multiple(&self.inner, &keys).await
    }
    ...
    pub async fn get_mobile_login_session(
        ...
        let key = format!("{}{}", MACRO_MOBILE_LOGIN_SESSION_PREFIX, session_code);
        conation_redis::get::get_optional::<String>(&self.inner, &key).await
    }
    ...
    pub async fn set_passwordless_login_code(&self, email: &str, code: &str) -> anyhow::Result<()> {
        let key = conation_passwordless_login_code!(email.to_lowercase());
    ...
    pub async fn get_passwordless_login_code(&self, email: &str) -> anyhow::Result<String> {
        let key = conation_passwordless_login_code!(email.to_lowercase());
        conation_redis::get::get::<String>(&self.inner, &key).await
    }
    ...
    pub async fn take_user_just_signed_up(&self, email: &str) -> anyhow::Result<bool> {
        let key = conation_just_signed_up!(email.to_lowercase());
        let value = conation_redis::get::get_del_optional::<u8>(&self.inner, &key).await?;
        Ok(value.is_some())
    }
}
