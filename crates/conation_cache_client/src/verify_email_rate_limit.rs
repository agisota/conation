...
impl MacroCache {
    ...
    pub async fn increment_resend_verify_email_rate_limits(
        ...
        conation_redis::incr::incr_with_expiry(&self.inner, &key_minute, MINUTE_EXPIRY_SECONDS)
            .await?;

        conation_redis::incr::incr_with_expiry(&self.inner, &key_day, DAY_EXPIRY_SECONDS).await?;
