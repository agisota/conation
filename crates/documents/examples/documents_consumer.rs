...
//! subscribes to the [`DocumentMacroEvent`] topic and prints every document
...
struct DocumentsConsumerGroup;

impl GroupName for DocumentsConsumerGroup {
    const GROUP_NAME: &'static str = "documents-consumer-example";
}

/// Bounded capacity of the channel between the poll loop and the processor.
const CHANNEL_CAPACITY: usize = 128;

conation_event_broker::declare_topics!(DeclaredMacroEvent: DocumentMacroEvent);

type DocumentsKafkaAdapter = KafkaConsumerAdapter<DocumentsConsumerGroup, DeclaredMacroEvent>;
type DocumentsConsumerService =
    MacroEventConsumerService<DeclaredMacroEvent, DocumentsKafkaAdapter>;
...
async fn main() -> anyhow::Result<()> {
    ...
    let consumer =
        KafkaEventConsumer::<DocumentsConsumerGroup>::from_env(env.kafka_brokers.as_ref())?;
    let consumer = KafkaConsumerAdapter::<DocumentsConsumerGroup, ()>::new(consumer)
        .subscribe::<DeclaredMacroEvent>()
        .map_err(|error| anyhow::anyhow!("failed to subscribe to documents topic: {error:?}"))?;
    let consumer = DocumentsConsumerService::new(consumer);
    println!(
        "listening on topic={} group={} brokers={} (ctrl-c to stop)",
        DeclaredMacroEvent::topics()[0],
        DocumentsConsumerGroup::GROUP_NAME,
        ...
        tokio::select! {
            ...
            result = consumer.recv() => {
                ...
                match message.decode_payload() {
                    Ok(DeclaredMacroEvent::DocumentMacroEvent(event)) => {
                        events_tx
                            .send(ReceivedEvent {
