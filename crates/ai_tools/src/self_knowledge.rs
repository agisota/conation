...
/// in an overview plus a routing map into the live docs at docs.conation.dev. Every
...
2. **Read the docs.** Conation's docs are published at docs.conation.dev. Every page is
...
3. **Cite the docs.** Link the user to the specific docs page you used, but always
   ...
   and strip any query string. Read `https://docs.conation.dev/getting-started.md`,
   but link `https://docs.conation.dev/getting-started`. Never route a user to a
   `.md` URL or to one carrying tracking parameters (e.g. `?_gl=…`) — surface only
   the clean route.

## How to find the right page

- **Full index of every page:** https://docs.conation.dev/llms.txt — fetch this
...
- Any page is readable as Markdown by appending `.md`, e.g.
  https://docs.conation.dev/product/tasks.md. To link that page for a user, use the
  route without the suffix: https://docs.conation.dev/product/tasks

### Quick links (fetch with a trailing `.md`; link users to the route without it)

- Get started: https://docs.conation.dev/getting-started.md
- Products (https://docs.conation.dev/product/<name>.md): email, channels, tasks,
  docs, canvas, calls, crm, folders, agents, inbox, search, snippets,
  unified-memory
- Concepts (https://docs.conation.dev/concepts/<name>.md): blocks, mentions,
  properties
- AI & MCP: https://docs.conation.dev/AI/mcp/overview.md ·
  tool reference https://docs.conation.dev/AI/mcp/tools/index.md ·
  recipes https://docs.conation.dev/AI/recipes.md
- Account: https://docs.conation.dev/account/billing.md ·
  https://docs.conation.dev/account/teams.md
- Other (https://docs.conation.dev/<name>.md): permissions, keyboard-shortcuts,
  faq, apps, support, integrations/github
- Changelog: https://docs.conation.dev/changelog/introduction.md

## Workflow for "what is Conation / how do I…" questions

1. Identify what the user is asking about (a block, a concept, billing, etc.).
2. Fetch the relevant docs.conation.dev `.md` page with WebFetch (or llms.txt first
...
/// docs at docs.conation.dev. It takes no arguments and does no I/O — it hands the
...
docs at docs.conation.dev; every docs page is readable as Markdown (append `.md` to its URL), \
