...
//! [`conation_env::Environment`]. The generated type also checks an override
...
/// override is not set, [`conation_env::Environment`] selects one of the provided
/// `local`, `dev`, or `prod` defaults.
///
/// # Example
///
/// ```
/// conation_queues::queue! {
///     #[derive(Debug, Clone)]
    ...
    (
        ...
        $crate::paste::paste! {
            ...
            impl $n {
                ...
                #[doc = "Default queue name for [`conation_env::Environment::Local`]."]
                $v const LOCAL: &'static str = $local;

                #[doc = "Default queue name for [`conation_env::Environment::Develop`]."]
                $v const DEV: &'static str = $dev;

                #[doc = "Default queue name for [`conation_env::Environment::Production`]."]
                ...
                $v fn new() -> Self {
                    Self::new_for_environment($crate::conation_env::Environment::new_or_prod())
                }

                #[doc = "Create a new instance of [`Self`] for a specific environment, using the override env var if set."]
                #[allow(dead_code)]
                $v fn new_for_environment(environment: $crate::conation_env::Environment) -> Self {
