...
async fn test_get_github_link_by_user_id(pool: Pool<Postgres>) {
    ...
    assert_eq!(link.macro_id.as_ref(), "conation|user@user.com");
...
async fn test_get_github_link_by_user_id_not_found(pool: Pool<Postgres>) {
    ...
    let user_id = MacroUserIdStr::parse_from_str("conation|nonexistent@user.com")
        .unwrap()
        .into_owned();
...
async fn test_insert_github_link(pool: Pool<Postgres>) {
    ...
    let link = GithubLink {
        ...
        macro_id: MacroUserIdStr::parse_from_str("conation|new@user.com")
            .unwrap()
            .into_owned(),
    ...
    assert_eq!(fetched.macro_id.as_ref(), "conation|new@user.com");
...
async fn test_insert_github_link_shared_github_user_id_succeeds(pool: Pool<Postgres>) {
    ...
    let link = GithubLink {
        ...
        macro_id: MacroUserIdStr::parse_from_str("conation|sharer@user.com")
            .unwrap()
            .into_owned(),
    ...
    assert_eq!(fetched.macro_id.as_ref(), "conation|sharer@user.com");
...
async fn test_insert_github_link_duplicate_conation_and_github_user_id_fails(pool: Pool<Postgres>) {
    let repo = PgGithubRepo::new(pool);
    ...
    let link = GithubLink {
        ...
        macro_id: MacroUserIdStr::parse_from_str("conation|user@user.com")
            .unwrap()
            .into_owned(),
        fusionauth_user_id: uuid::Uuid::new_v4(),
