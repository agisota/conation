...
#[test]
fn load_uses_conation_config_default_value_when_key_is_missing() {
    let _lock = ENV_LOCK.lock().expect("env lock poisoned");
...
#[test]
fn load_uses_config_value_over_conation_config_default_value() {
    let _lock = ENV_LOCK.lock().expect("env lock poisoned");
...
#[test]
fn load_deserializes_conation_env_var_fields() {
    let _lock = ENV_LOCK.lock().expect("env lock poisoned");
    let env = EnvGuard::new(&[
        "APP_SECRETS_JSON",
        "CONFIG_SECRET",
...
#[test]
fn load_deserializes_local_or_remote_conation_env_var_fields() {
    let _lock = ENV_LOCK.lock().expect("env lock poisoned");
    let env = EnvGuard::new(&[
        "APP_SECRETS_JSON",
...
#[tokio::test]
async fn conation_config_derived_resolve_remote_secrets_resolves_local_or_remote_fields() {
    let _lock = ENV_LOCK.lock().expect("env lock poisoned");
    let env = EnvGuard::new(&[
        "APP_SECRETS_JSON",
        "REMOTE_CONFIG_SECRET",
    ...
    let config = ConfigLoader::load::<RemoteSecretFieldConfig>().expect("config should load");
    let config = config
        .resolve_remote_secrets(conation_env::Environment::Develop, &TestSecretManager)
        .await
