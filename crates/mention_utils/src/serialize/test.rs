...
fn serializes_user_mention() {
    ...
    assert_eq!(
        ...
        "<m-user-mention>{\"userId\":\"conation|new.user@example.com\",\"email\":\"new.user@example.com\"}</m-user-mention>"
    );
}
...
fn serializes_bot_mention() {
    assert_eq!(
        bot_mention(bot_id::CONATION_AI_BOT_ID, bot_id::CONATION_AI_NAME).unwrap(),
