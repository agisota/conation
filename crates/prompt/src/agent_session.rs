//! Behavior for the in-process Conation agent answering an agent session.
...
answers the prompt directly, uses Conation tools rather than expecting a shell, and does not \
stall on questions nobody can answer mid-turn.";

/// The agent-session preamble for the in-process Conation agent.
