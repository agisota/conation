//! Rules for linking to Conation items in the model's own *conversational
//! replies* sent to external MCP clients.
//!
//! Unlike the in-app [`crate::mentions`] section — which tells the model to emit
//! `<m-document-mention>` XML tags that only render inside the Conation app — MCP
...
//! the model writes *into* a Conation document (e.g. via `CreateDocument` or
...
//! is rendered by the Conation app itself, not by the MCP client.
//!
...
static TITLE: &str = "Linking to and listing Conation items";
...
pub fn render(base_url: &str) -> String {
    format!(
        "# {TITLE}\n\
         When referring the user to a Conation item (document, channel, chat, \
         ...
         \n\
         Do NOT emit `<m-document-mention>` XML tags or any other Conation internal \
         mention/markup format in these responses. Those only render inside the \
         Conation app and appear as raw text to MCP clients. This rule overrides any \
         ...
         \n\
         This rule does NOT apply to content you write into a Conation document, \
         e.g. via the `CreateDocument` or `EditDocument` tools. That content is \
         rendered by the Conation app itself, not by this MCP client, so it must \
         still use `<m-document-mention>` tags to link correctly — see \"Linking \
         Conation items inside document content\" below.\n\
         \n\
         When you list multiple Conation items, present them as a Markdown table with \
