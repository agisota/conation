...
/// The global text-map propagator is registered by `conation_entrypoint`.
...
/// propagator is registered by `conation_entrypoint`; without one, or outside any
...
const HTTP_REQUEST_SPAN_TARGET: &str = "conation_http_request";
...
/// `conation_http_request=info` to `RUST_LOG` when log events should include these span fields.
