...
#[allow(
    ...
    reason = "conation_env_var is the sanctioned wrapper around std::env::var"
)]
...
/// # Deserialization
///
/// When deserialized (for example as a field of a `conation_config::MacroConfig` struct), a missing
...
macro_rules! maybe_env_var {
    (
        ...
        $crate::paste::paste! {
            ...
            When deserialized (e.g. by `conation_config`), a missing variable produces [`" $n "`::Unset]
            ...
            value may be unset (e.g. when loaded through `conation_config`)."]
            impl std::ops::Deref for $n {
                type Target = str;
