...
fn render(locale: SupportedLocale, url: &Url) -> RenderedVerificationEmail {
    render_verification_email(
        ...
        VerificationEmail {
            ...
            support_email: "pythia@conation.dev",
        },
    )
...
fn renders_complete_english_source_copy() {
    ...
    assert!(rendered.html().contains("mailto:pythia@conation.dev"));
    assert!(rendered.html().contains(">pythia@conation.dev</a"));
}
...
#[test]
fn renders_an_operator_owned_support_address() {
    let url = Url::parse("https://auth.workspace.example/email/verify/123").unwrap();
    let rendered = render_verification_email(
        SupportedLocale::Russian,
        VerificationEmail {
            verification_url: &url,
            support_email: "help@workspace.example",
        },
    )
    .unwrap();
    ...
    assert!(
        rendered
            .html()
            .contains("href=\"mailto:help@workspace.example\"")
    );
    assert!(!rendered.html().contains("@macro.com"));
    assert!(!rendered.html().contains("@conation.dev"));
}
