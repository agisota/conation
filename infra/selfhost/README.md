...
The Docker volumes `macro_postgres_data`, `macro_redis_data`,
`macro_opensearch_data`, and `macro_kafka_data` deliberately retain their
pre-rebrand names. They are persisted compatibility identifiers, not display
branding. The tooling does not copy, rename, or delete either these volumes or
any `conation_*` volumes an earlier experimental build may have created; an
operator must inspect and migrate data explicitly before adopting a different
volume.
...
| EventBridge/scheduled jobs | Individual local workers may be runnable | Complete schedule inventory, leader election, and missed-run recovery are unsupported |
...
backend: account provisioning, credentials, JMAP reads, sync cursors, push
events, outbound authentication, and migration have not been wired and tested.
No custom Stalwart configuration is shipped or mounted until those lifecycle,
secret-injection, and integration paths have an executable validation gate.
...
3. Replace or formally exclude SNS, Secrets Manager, Lambda, ECS, schedules,
   and every remaining direct AWS client path.
