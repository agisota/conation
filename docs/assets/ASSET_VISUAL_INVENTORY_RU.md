...
### 11. `apps/web/src/components/icon/macro-brand-loader-2.svg`

- Техника: SVG, 37×20, 5,120 B, alpha: да; SMIL-анимация без дискретных кадров, три группы с периодом 4 s и задержками 0/0.2/0.4 s; SHA-256 `dfdd9087a850494bb1b5a33794b5fec8aabfbc62b7d3467b592c1a1e99b3215d`.
- Назначение: `LoadingSpinner.tsx`.
- Вид/OCR: три изометрических полупрозрачных столбца/ленты, которые последовательно скользят через пунктирные основания; текста нет.
- Тема/a11y: `currentColor` адаптируется к теме, но бесконечное движение должно отключаться при `prefers-reduced-motion` и сопровождаться текстовым статусом.
- Происхождение/лицензия: **U**; имя и геометрия всё ещё Macro-branded.
