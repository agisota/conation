...
describe('comment mark removal', () => {
  ...
  it('ignores detached leftovers that still carry the ID', () => {
    const { editor, removed } = setup();
    const key = addRange(editor);
    let leftover: CommentNode | undefined;
    editor.update(
      () => {
        leftover = $createCommentNode({ ids: ['mark'], isDraft: false });
      },
      { discrete: true }
    );
    // Selection deletes in the collaborative editor leave detached mark nodes
    // in the committed node map, which read-mode `$nodesOfType` still lists.
