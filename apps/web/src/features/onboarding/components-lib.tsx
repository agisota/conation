...
export function ContinueButton(props: ContinueButtonProps) {
  return (
    <button
      ...
      type="button"
      class={cn(
        'w-full px-3 py-2.5 text-lg font-bold rounded-xs flex items-center gap-2 border border-transparent',
        props.centered ? 'justify-center' : 'justify-between',
        {
          'bg-transparent text-ink/40 font-normal border border-edge-muted':
            props.disabled,
