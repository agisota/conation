...
const TABS: SettingsTabItem[] = [
  ...
  { tab: 'Billing', label: 'Access', icon: Icon },
...
describe('searchSettings', function () {
  ...
  it('finds top-level pages by their sidebar label, best match first', function () {
    ...
    expect(titles('Access')[0]).toBe('Access');
  ...
  it('covers the MCP server page and Access tokens', function () {
    expect(tabs('api key')).toContain('Billing');
    expect(tabs('token')).toContain('Billing');
    expect(tabs('mcp')).toContain('Agent');
    expect(titles('mcp server')[0]).toBe('MCP server');
  });
