...
/**
 * `openExternalUrl` interceptor: inside the native Tauri shell, routes a Conation
 ...
 *
 * Returns false for non-Tauri and foreign `/app` URLs so `openExternalUrl`
 * keeps handling them.
 */
export function openMacroLinkInApp(url: string): boolean {
  ...
  emit('navigate', parsed).catch((e) => {
    console.error('failed to emit navigate event for Conation link', e);
