import { DocxExpandedPartList } from "@service-storage/util/getDocxFile";
...
async function getOPFSDocumentDirHandle() {
  if (!opfsDocumentDirHandle_) {
    ...
    opfsDocumentDirHandle_ = await opfsHandle.getDirectoryHandle(OPFS_DIR_NAME, {
      create: true,
    });
  }
...
export class OPFSDocument {
  ...
  async addPart(path: string, sha: string, buffer: ArrayBuffer): Promise<void> {
    const fullPath = `${encodeURIComponent(path)}@${sha}`;
